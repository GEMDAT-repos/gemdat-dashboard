...
st.set_page_config(
    ...
    page_icon='💎',
    menu_items={
        'Get Help': 'https://gemdat.readthedocs.io',
        'Report a bug': 'https://github.com/gemdat-repos/gemdat/issues',
        'About': (
...
with st.sidebar:
    ...
    diffusing_element = st.selectbox('Diffusive Element', elements, index=index)
    ...
    sites_filename = str(st.selectbox('Load sites from known material', KNOWN_MATERIALS))
    ...
    if manual_supercell:
        ...
        supercell = (
            ...
            int(
                col2.number_input(
                    'supercell y', min_value=1, value=1, label_visibility='collapsed'
                )
            ),
            int(
                col3.number_input(
                    'supercell z', min_value=1, value=1, label_visibility='collapsed'
                )
            ),
        )
    ...
    if do_error:
        with st.sidebar:
            n_parts = int(st.number_input('Number of parts to divide trajectory in', value=10))

    st.markdown('## Radial distribution function')
    do_rdf = st.checkbox('Plot RDFs')
    if do_rdf:
        with st.sidebar:
            max_dist_rdf = st.number_input('Maximum distance for RDF calculation', value=10.0)
            resolution_rdf = st.number_input('Resolution (width of bins)', value=0.1)
...
metrics_std = SimulationMetricsStd(diff_trajectory.split(n_parts, equal_parts=True))
...
with col1:
    ...
    st.metric('Attempt frequency ($\\mathrm{s^{-1}}$)', value=f'{attempt_freq:g}')
    st.metric(
        'Vibration amplitude ($\\mathrm{Å}$)',
        value=f'{metrics_std.vibration_amplitude():g}',
    )
with col2:
    st.metric('Particle density ($\\mathrm{m^{-3}}$)', value=f'{metrics.particle_density():g}')
    st.metric('Mol per liter ($\\mathrm{mol/l}$)', value=f'{metrics.mol_per_liter():g}')
with col3:
    st.metric(
        'Tracer diffusivity ($\\mathrm{m^2/s}$)',
...
if manual_supercell:
    ...
    zipped_parameters = zip(trajectory.get_lattice().abc, sites_structure.lattice.parameters)
...
if not is_lattice_similar(trajectory.get_lattice(), sites_structure):
    ...
    st.text(f'{trajectory_location.name}: {trajectory.get_lattice().parameters}')
    st.stop()
...
@st.cache_data(hash_funcs={Structure: _structure_hash_func, Trajectory: _trajectory_hash_func})
...
with tab1:
    ...
    figures = (
        ...
        plots.vibrational_amplitudes(trajectory=diff_trajectory, n_parts=n_parts),
...
if do_rdf:
    with tab2:
        ...
        rdf_figures = [plots.radial_distribution(rdfs) for rdfs in rdf_data.values()]
...
with tab3:
    ...
    background_level = col2.number_input(
        ...
        step=0.05,
        help=(
            'Fraction of the maximum volume value '
            'to set as the minimum value for peak segmentation.'
        ),
    )
    ...
    @st.cache_data(hash_funcs={Volume: _volume_hash_func, Structure: _structure_hash_func})
